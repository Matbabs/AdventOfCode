...
int main(void) {
    ...
    while(numbers.size()){
        bool isWinning = false, isLastGrid = false;
        ...
        for(auto& grid : grids){
            ...
            auto checkLastWin = [&](bool mode){
                for(int i = 0; i < SIZE; i++){
                    ...
                    if(isWinning) {
                        grids_win.insert(nb_grid);
                        if(grids_win.size() == grids.size()){
                            isLastGrid = true; break;
                        }  
                    }
                }
                return isLastGrid;
            };
            if(checkLastWin(true)) break;
            if(checkLastWin(false)) break;

...
bool isReachable(int vx, int vy, int *xt, int *yt, int &max_y){
    int x = 0, y = 0;
    while(x < xt[1] && y > yt[0]){
        x += vx; y += vy;
        if(vx != 0) vx += vx > 0 ? -1 : 1;
...
int main(void) {
    ...
    for(int i = -BRUTE_FORCE; i < BRUTE_FORCE; i++)
        for(int j = 0; j < BRUTE_FORCE; j++){
            int max_y = 0;
            if(isReachable(j, i, xt, yt, max_y))
                possibles.push_back(max_y);
        }
    cout << *max_element(possibles.begin(), possibles.end()) << endl;
    return 0;
}
...
func main() {
	...
	cd := map[string]int{"J": 0, "2": 1, "3": 2, "4": 3, "5": 4, "6": 5, "7": 6, "8": 7, "9": 8, "T": 9, "Q": 10, "K": 11, "A": 12}
	cds := make([][]string, 0)
	...
	for scanner.Scan() {
		...
		maxH := "o"
		if strings.Contains(c[0], "J") {
			for _, r := range c[0] {
				cr := strings.Replace(c[0], "J", string(r), -1)
				if tp[maxH] < tp[hand(cr)] {
					maxH = hand(cr)
				}
			}
		} else {
